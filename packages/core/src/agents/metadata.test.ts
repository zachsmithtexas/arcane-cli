...
describe('MetadataManager', () => {
  ...
  describe('agent directory creation', () => {
    ...
    it('should generate proper metadata file with YAML frontmatter', async () => {
      ...
      const metaCall = mockFs.writeFile.mock.calls.find((call: [string]) =>
  ...
  describe('role directory creation', () => {
    it('should create role directory with proper structure', async () => {
      const role: Role = {
        ...
        allowedTools: ['read-files', 'write-reports'],
        restrictedTools: ['no-production-access'],
        priority: Priority.MEDIUM,
        created: '2025-01-01T00:00:00.000Z',
        version: '1.0.0',
        tags: ['testing'],
  ...
  describe('skill directory creation', () => {
    it('should create skill directory with proper structure', async () => {
      const skill: Skill = {
        ...
        allowedTools: ['jest', 'vitest'],
        restrictedTools: ['no-production-testing'],
  ...
  describe('metadata loading', () => {
    ...
    it('should return null for non-existent agent', async () => {
      ...
      const agent =
        await metadataManager.loadAgentMetadata('Non-existent Agent');

      expect(agent).toBeNull();
    });

    it('should handle malformed metadata gracefully', async () => {
      const malformedContent = `---invalid yaml content---Test content`;
  ...
  describe('metadata updating', () => {
    it('should update agent metadata file with new timestamp', async () => {
      ...
      expect(new Date(agent.updated!).getTime()).toBeGreaterThan(
  ...
  describe('frontmatter parsing', () => {
    it('should parse simple YAML frontmatter correctly', async () => {
      ...
      const result = metadataManager.parseFrontmatter(content);
  ...
  describe('YAML generation', () => {
    it('should generate proper YAML from objects', async () => {
      ...
      // @ts-expect-error Testing private method
      const yaml = metadataManager.objectToYaml(obj);
